...
const PackageCards: React.FC = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/package/published")
      .then((res) => res.json())
      .then((data: Package[]) => {
        const validPackages = data.filter(pkg => pkg !== null && typeof pkg === "object");
        setPublished(validPackages);
      })
      .catch((err) => console.error("Error fetching published packages:", err));
  }, []);
  ...
  const renderCards = (packages: Package[]) =>
    packages.map((pkg) => {
      const imageSrc =
        pkg.cover_image && pkg.cover_image !== "null" && pkg.cover_image.trim() !== ""
          ? pkg.cover_image
...
{pkg.feeDetails && !pkg.is_free && (
  <div className="card-price">
    {Number(pkg.feeDetails.discount_value) > 0 ? (
      <>
  ...
  // Filter packages based on the filter state
      ...
      <div className="page-layout">
        <div className="grid-container">
          <div className="package-grid">
            {filteredPackages(filteredPublished).length > 0
              ? renderCards(filteredPackages(filteredPublished))
              : <p>No published packages available.</p>}
          </div>
        </div>
      </div>
    </div>
